...
function showSeatDetails(pos) {
    ...
    elPopup.querySelector('h3 .price').innerText = `$${seat.price}`
    elPopup.querySelector('h3 .seats-around').innerText = getMsg(getAvailableSeatsAround(pos))
...
function getAvailableSeatsAround(pos) {
    ...
    for (var i = pos.i - 1; i <= pos.i + 1; i++) {
        ...
        for (var j = pos.j - 1; j <= pos.j + 1; j++) {
            ...
            if (gCinema[i][j].type === 'S' && !gCinema[i][j].isBooked) emptsSeats.push({ i, j })
        }
    }
...
function getMsg(emptsSeats) {

    var resStr = ''

    if (emptsSeats.length === 0) {

        resStr = 'There are no available seats around'

    } else {
        for (var i = 0; i < emptsSeats.length; i++) {
            resStr += `Seat-${emptsSeats[i].i}-${emptsSeats[i].j}, `
        }

        resStr = resStr.slice(0, resStr.length - 2);
